...
def get_args_parser():
    ...
    parser.add_argument('--batch_size', default=110, type=int, metavar='N',
                        help='number of samples per batch')
    parser.add_argument('--savedir', '-s', metavar='DIR',
                        default='../checkpoint/quickdraw/',
...
def train(train_loader, train_loader_ext, model, criterion, criterion_kd, optimizer, model_t, criterion_kd_1,
          optimizer_1, epoch):
    ...
    for i, ((input, target), (input_ext, target_ext)) in enumerate(zip(train_loader, train_loader_ext)):
        ...
        if epoch <= 3:
            ...
            loss_kd_1 = criterion_kd(output_kd, output_t, tag_all)
            ...
            output_t = model_t(input_all)
            loss_kd_1 = criterion_kd(output_kd, output_t, tag_all)
            loss_kd_2 = criterion_kd_1(output_t, output_kd, tag_all)
...
quickddraw_train = QuickDrawDataset(split='train', zero_version=args.zero_version, transform=transformations, aug=True, cid_mask=False)
train_loader = DataLoader(dataset=quickddraw_train, batch_size=args.batch_size // 10, shuffle=True, num_workers=2)

quickdraw_train_ext = QuickDrawDataset(split='train', zero_version=args.zero_version, version='all_photo', transform=transformations, aug=True, cid_mask=False)
train_loader_ext = DataLoader(dataset=quickdraw_train_ext, batch_size=args.batch_size // 10 * 9, shuffle=True, num_workers=2)
...
for epoch in range(args.epochs):
    ...
    }, warmup_epoch, is_best=None, filename=os.path.join(args.savedir, 'checkpoint.pth.tar'))
