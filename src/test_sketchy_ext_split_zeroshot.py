...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume_dir', '-s', metavar='DIR', default='../checkpoint/sketchy/zeroshot2')
    parser.add_argument('--zero_version', metavar='VERSION', default='zeroshot2', type=str,
                        help='zeroshot version for training and testing (default: zeroshot2)')
    parser.add_argument('--batch_size', default=80, type=int, metavar='N',
                        help='number of samples per batch')
    parser.add_argument('--nb_classes', default=104, type=int,
    ...
    if args.mode == 2:
        ...
        for M in Ms:
            for K in Ks:
                ...
                for fi in range(predicted_features_query.shape[0]):
                    mapi_binary = eval_AP_inner(gt_labels_query[fi], binary_scores[fi], gt_labels_gallery, top=200)
        ...
        for fi in range(predicted_features_query.shape[0]):
            mapi = eval_AP_inner(gt_labels_query[fi], scores[fi], gt_labels_gallery, top=200)
