...
def get_args_parser():
    ...
    parser.add_argument('--savedir', '-s', metavar='DIR', default='../checkpoint/tuberlin/', help='path to save dir')
...
def adjust_learning_rate(optimizer, epoch, flag):
    if flag == 0:
        ...
        lr = (args.lr / 2) * math.pow(0.001, float(epoch) / args.epochs)
...
if args.finetune and not args.eval:
    ...
    trash_vars = [k for k in checkpoint_model.keys() if k not in model_dict.keys()]
...
optimizer_1 = torch.optim.Adam(model_t.parameters(), lr=args.lr / 2, weight_decay=args.weight_decay)
...
for epoch in range(args.epochs):
    model.train()
    adjust_learning_rate(optimizer, epoch, flag=0)
    adjust_learning_rate(optimizer_1, epoch, flag=1)
