...
def prepare_features():
    ...
    quickdraw_zero_ext = QuickDrawDataset(split='zero', version='all_photo', zero_version=args.zero_version,
                                          transform=transformations, aug=False)

    zero_loader_ext = DataLoader(dataset=quickdraw_zero_ext, batch_size=args.batch_size, shuffle=False, num_workers=0)

    quickdraw_zero = QuickDrawDataset(split='zero', zero_version=args.zero_version, transform=transformations,
                                      aug=False)
    zero_loader = DataLoader(dataset=quickdraw_zero, batch_size=args.batch_size, shuffle=False, num_workers=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume_dir', '-s', metavar='DIR', default='../checkpoint/quickdraw')
    ...
    if args.mode == 2:
        ...
        for M in Ms:
            for K in Ks:
                ...
                for fi in range(predicted_features_query.shape[0]):
                    prec_binary = eval_precision(gt_labels_query[fi], binary_scores[fi], gt_labels_gallery, top=200)
        ...
        for fi in range(predicted_features_query.shape[0]):
            prec = eval_precision(gt_labels_query[fi], scores[fi], gt_labels_gallery, top=200)
